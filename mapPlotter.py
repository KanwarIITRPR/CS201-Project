...
def extractDivisions():
    contents = []
    with open(divisions_file, "r") as file:
        contents = list(map(lambda x: list(map(lambda y: float(y), x.removesuffix("\n").strip().split(" "))), file.readlines()))

    points, depths = [], []
    for hospital_idx in range(len(contents)):
        points.append(contents[hospital_idx][:-1])
        depths.append(contents[hospital_idx][-1])
